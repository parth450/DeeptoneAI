...
export default function FileUpload({ username, refreshHistory, onUploadComplete }) {
  ...
  const handleAnalyze = async () => {
    ...
    if (!username) return alert('You must login first.');
    ...
    try {
      ...
      alert('❌ Failed to analyze. Try again after 30–60 sec.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="flex flex-col items-center p-8 rounded-lg shadow-xl w-full max-w-2xl mx-auto text-white" style={{ backgroundColor: '#0f172a' }}>
      <label className="border-2 border-dashed border-teal-400 p-6 w-full text-center rounded-md cursor-pointer mb-4 hover:bg-slate-800 transition">
        Choose Audio
        <input type="file" accept=".wav,.mp3,.m4a" className="hidden" onChange={handleFileChange} />
      </label>

      {previewURL && <audio className="mb-4" controls src={previewURL}></audio>}
      ...
      <p className="text-sm text-gray-300 mt-2">Supported: WAV, MP3, M4A (Max 100MB)</p>
